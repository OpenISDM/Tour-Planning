...
public class Circle {
	...
	*/
	public void main(Circle original)
	{
	...
	*/
	public void inTurnClustering()
	{
		...
		for(int i=1;i<data.SALESMAN;i++)
			dead[i] = false;
		initSales();
		// sort the global variable list for all participants
		// list contains the ID of nodes in a 
		data.sortList();
		...
		while(data.copies>0)
		{
			...
			for(int i=1 ; i<data.SALESMAN ; i++)
			{
				...
				for(int j=1 ; j<data.TOTALNODES+1 ; j++)
				{
					...
					dist = dist(data.nodes[data.list[i][j]][0],data.nodes[data.list[i][j]][1],data.depot[i][0],data.depot[i][1]);
			...
			// while all the left nodes are too far for the 
			if (data.copies>0 && dcount==data.SALESMAN-1)
			{
				...
				for(int i=1 ; i<data.TOTALNODES+1 ; i++)
				{
					...
					for(int j=1 ; j<data.SALESMAN ; j++)
					{
						...
						dist = dist(data.nodes[i][0],data.nodes[i][1],data.depot[j][0] , data.depot[j][1]);
						if(dist<min)
						{
							min = dist;
							whichMin = j;
