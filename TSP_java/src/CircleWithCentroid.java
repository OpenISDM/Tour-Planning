...
public class CircleWithCentroid {
	...
	public CircleWithCentroid(Data data)
	{
		this.data = data;
	}
	...
	*/
	public void main(CircleWithCentroid original)
	{
		...
		while(data.copies>0)
		{
			...
			for(int i=1 ; i<data.SALESMAN ; i++)
			{
				...
				for(int j=1 ; j<data.TOTALNODES+1 ; j++)
				{
					...
					dist = dist(data.nodes[data.list[i][j]][0],data.nodes[data.list[i][j]][1],data.centroids[i][0],data.centroids[i][1]);
			...
			// nodes outside the circle!!
			// while all the left nodes are too far for the centroid
			if (data.copies>0 && dcount==data.SALESMAN-1)
			{
				...
				for(int i=1 ; i<data.TOTALNODES+1 ; i++)
				{
					...
					for(int j=1 ; j<data.SALESMAN ; j++)
					{
						...
						dist = dist(data.nodes[i][0],data.nodes[i][1],data.centroids[j][0] , data.centroids[j][1]);
						if(dist<min)
						{
							min = dist;
							whichMin = j;
	...
	public void determineDvalue()
	{
		...
		for(int i=1;i<data.SALESMAN;i++)
		{
			...
			for(int j=1 ; j<data.SALESMAN ; j++)
			{
				if (i==j)
					continue;
				distt = dist(data.centroids[i][0],data.centroids[i][1],data.centroids[j][0],data.centroids[j][1]);
				if(distt<min)
				{
					min = distt;
				}
